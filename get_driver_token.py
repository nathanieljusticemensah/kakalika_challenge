"""Create or reuse a test driver and print an access token.
...
def create_test_driver_user(admin_client: object, phone: str) -> dict:
    """Create a test driver user using the admin service role client."""
    ...
    if not user:
        raise RuntimeError("Failed to create test driver user")
    return user
...
def main() -> None:
    phone = os.getenv("TEST_DRIVER_PHONE")
    otp_code = os.getenv("TEST_DRIVER_OTP_CODE")

    if not phone:
        print("ERROR: Set TEST_DRIVER_PHONE in .env to your registered test phone number.")
        sys.exit(1)
    if not otp_code:
        print("ERROR: Set TEST_DRIVER_OTP_CODE in .env to the fixed OTP code configured for that number.")
        sys.exit(1)
    ...
    if user:
        ...
        print(f"Creating new test driver user for phone {phone}")
        user = create_test_driver_user(admin_client, phone)
        user_id = getattr(user, "id", None) or (user.get("id") if isinstance(user, dict) else None)
